export default class Unit {
  /* constructor(unitId, faction, type, name) {
    this.id = unitId;
    ...
    this.unitInfo = factionList[faction].units[type][name];
  ...
  setupDefaultModels() {
    for (var modelType in this.unitInfo.modelTypes) {
      let model = this.unitInfo.modelTypes[modelType].model;
      this.models[model] = [];
      let modelInfo = factionList[this.faction].models[model];
      if (modelInfo !== undefined) {
        for (var num = 0; num < this.unitInfo.modelTypes[modelType].defaultAmount; num++) {
        ...
        if (!model.modelInfo.PointsIncludesWargear) {
          if(model.modelInfo.Wargear.weapons) {
            ...
            for (let weapon = 0; weapon < selectedWeapons.length; weapon++) {
              points += factionList[this.faction].wargear[selectedWeapons[weapon]].Points;
            }
          }
        }
      }
  ...
  getUnitNameAndCostHTML() {
    var costHTML =  'Points: ' + this.totalPoints
